...
export function AuthService($location, $http, $cookies, $q, appConfig, Util, User, Owner, TeamLeader) {
  'ngInject';
  ...
  var Auth = {
    ...
    createTeam(orgTeam,  callback ? : Function) {
      return Owner.createTeam({
...
// Delete a particular team
    deleteTeam(orgTeam,  callback ? : Function) {
      return Owner.deleteTeam({
        email: currentUser.email
      }, {
        orgTeam
      }, function() {
    ...
    addTeamMember(team,  callback ? : Function) {
      return TeamLeader.addTeamMember({
        email: currentUser.email
      }, {
        team
      }, function() {
     ...
     */
    isOwner() {
      return Auth.hasRole.apply(Auth, [].concat.apply(['owner'], arguments));
    },
     ...
     */
    isOwnerSync() {
      return Auth.hasRoleSync('owner');
    },


    /**
     * Check if a user is a teamleader
     *   (synchronous|asynchronous)
     *
     * @param  {Function|*} callback - optional, function(is)
     * @return {Bool|Promise}
     */
    isTeamleader() {
      return Auth.hasRole.apply(Auth, [].concat.apply(['thead'], arguments));
    },
